...
engine = create_engine("postgresql://postgres:boc5978@localhost:5432/pubsdb")
...
def write_to_db(res):
    with Session(engine) as session:
        for pub in res:
            try:
                session.add(Pub(name=pub["name"], address=pub["address"], postcode=pub["postcode"], lat=pub["lat"], lng=pub["lng"]))
                session.commit()
